...
import { AIAnalysisResult, RejectionInsightsResult, Job } from "./types";
...
function parseAnalysisResponse(response: string): AIAnalysisResult {
    try {
        ...
        try {
            ...
            console.error(
                "JSON parse error, attempting to fix truncated response:",
                parseError,
            );
            ...
            if (lastCompleteField > 0) {
                const fixedJson =
                    jsonStr.substring(0, lastCompleteField + 1) + "\n}";
        ...
        const result: AIAnalysisResult = {
            ...
            workArrangement:
                typeof parsed.workArrangement === "string"
                    ? parsed.workArrangement
                    : null,
            suitabilityScore:
                typeof parsed.suitabilityScore === "number"
                    ? parsed.suitabilityScore
                    : null,
            suitabilityReason:
                typeof parsed.suitabilityReason === "string"
                    ? parsed.suitabilityReason
                    : null,
        ...
        if (
            ...
            result.suitabilityReason =
                "Analysis was truncated - please re-analyze for complete scoring.";
        }
...
export async function analyzeRejectionPatterns(
...
): Promise<RejectionInsightsResult> {
    if (!process.env.ANTHROPIC_API_KEY) {
        throw new Error("ANTHROPIC_API_KEY is not configured");
    }
    ...
    const jobSummaries = jobs
        .map((job, i) => {
            const parts = [`Role ${i + 1}: ${job.title} at ${job.company}`];
            ...
            if (job.suitabilityScore != null)
                parts.push(`AI Suitability Score: ${job.suitabilityScore}/100`);
            if (job.suitabilityReason)
                parts.push(`AI Match Reasoning: ${job.suitabilityReason}`);
            return parts.join("\n");
        })
        .join("\n\n---\n\n");

    const cvSection = cvContent
        ? `\n\nCandidate CV:\n"""\n${cvContent}\n"""\n\nUse this CV to identify specific skill gaps and mismatches relative to these rejected roles.`
        : "";

    const prompt = `You are analyzing ${jobs.length} job applications that were all REJECTED. Your goal is to find patterns and common themes across these rejections to help the candidate understand why they may be getting rejected and how to improve.

Here are the rejected job applications:

${jobSummaries}
${cvSection}

Analyze all of these rejected roles and respond in JSON format:

{
  "commonRequirements": string[] (5-8 skills/requirements that appear most frequently across these rejected roles),
  "skillGaps": string[] (3-6 specific skill gaps or mismatches that likely contributed to rejections${cvContent ? ", based on the CV provided" : ""}),
  "patterns": string[] (3-5 patterns you notice - e.g. seniority level mismatch, industry clustering, role type trends, location patterns),
  "recommendations": string[] (4-6 specific, actionable steps the candidate should take to improve their success rate),
  "summary": string (2-3 sentence overview of the key takeaway from analyzing these rejections)
}

IMPORTANT:
- Be specific and reference actual data from the roles
- Focus on actionable insights, not generic advice
- If roles share common requirements the candidate may lack, highlight those specifically
- Look for patterns in role seniority, industry, tech stack, or company size`;
    ...
    try {
        ...
        return {
            ...
            commonRequirements: Array.isArray(parsed.commonRequirements)
                ? parsed.commonRequirements
                : [],
        ...
        console.error("Rejection pattern analysis failed:", error);
        ...
        throw new Error(
            "Failed to analyze rejection patterns. Please try again.",
        );
    }
}
