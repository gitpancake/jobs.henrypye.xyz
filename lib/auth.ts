...
async function resolveToken(token: string): Promise<SessionUser | null> {
  try {
    ...
    const sharedUser = await prisma.sharedUser.upsert({
      ...
      create: { firebaseUid: decoded.uid, email, displayName, photoUrl: photoURL },
      update: { email, displayName, photoUrl: photoURL },
    });
    ...
    if (memberships.length === 0) {
      await prisma.sharedTeam.create({
        data: {
          ...
          members: { create: { userId: sharedUser.id, role: "owner" } },
        },
      });
